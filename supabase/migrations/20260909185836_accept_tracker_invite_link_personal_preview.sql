-- Preview only: complete recipient personal linkage in the acceptance transaction.
BEGIN;
CREATE OR REPLACE FUNCTION public.accept_tracker_invite_transactional(
  p_org_id uuid,
  p_invite_token text,
...
DECLARE
  ...
  v_person public.personal%ROWTYPE;
  v_owner uuid;
  v_user uuid;
  v_hash text;
  v_email text;
...
BEGIN
  ...
  IF p_expected_user_id IS NOT NULL AND p_expected_user_id <> v_user THEN
    RAISE EXCEPTION 'invite_identity_mismatch' USING ERRCODE='P0001';
  END IF;
  -- Lock the recipient row before checking it; concurrent edits cannot replace identity.
  -- No personal row remains supported for existing backend-only invitations.
  BEGIN
    SELECT p.* INTO STRICT v_person FROM public.personal p
    WHERE p.org_id=p_org_id AND coalesce(p.is_deleted,false)=false
      AND lower(btrim(coalesce(nullif(btrim(p.email_norm),''),p.email,'')))=v_email
    FOR UPDATE;
  EXCEPTION
    WHEN no_data_found THEN v_person := NULL;
    WHEN too_many_rows THEN
      RAISE EXCEPTION 'invite_identity_ambiguous' USING ERRCODE='P0001';
  END;
  IF v_person.id IS NOT NULL AND (
    lower(btrim(v_person.email)) IS DISTINCT FROM v_email
    OR (v_person.user_id IS NOT NULL AND v_person.user_id<>v_user)
    OR EXISTS (SELECT 1 FROM public.personal p WHERE p.org_id=p_org_id
      AND coalesce(p.is_deleted,false)=false AND p.user_id=v_user AND p.id<>v_person.id)
  ) THEN
    RAISE EXCEPTION 'invite_identity_mismatch' USING ERRCODE='P0001';
  END IF;
  ...
  IF v_invite.expires_at <= v_now THEN
    RAISE EXCEPTION 'invite_expired' USING ERRCODE='P0001';
  END IF;
  -- Link only after all authorization/cap checks, also on a valid retry.
  -- Failure later in acceptance rolls this update back with membership/bridges.
  IF v_person.id IS NOT NULL AND v_person.user_id IS NULL THEN
    BEGIN
      UPDATE public.personal SET user_id=v_user
      WHERE id=v_person.id AND org_id=p_org_id AND user_id IS NULL;
      IF NOT FOUND THEN
        RAISE EXCEPTION 'invite_identity_mismatch' USING ERRCODE='P0001';
      END IF;
