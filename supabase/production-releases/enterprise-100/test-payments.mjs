...
const env={PADDLE_ENV:'live',PADDLE_API_KEY_LIVE:'sandbox-test-only',PADDLE_PRO_PRICE_ID_LIVE:'pri_test_pro',PADDLE_ENTERPRISE_PRICE_ID_LIVE:'pri_test_enterprise',PADDLE_ENTERPRISE_100_PRICE_ID_LIVE:'pri_test_enterprise100'};
function load(slug,overrides={}){
 let handler;
 const source=fs.readFileSync(`${root}/functions/${slug}/index.ts`,'utf8').replace(/^import .*;\r?\n/gm,'');
...
delete env.PADDLE_ENTERPRISE_100_PRICE_ID_LIVE;
...
env.PADDLE_ENTERPRISE_100_PRICE_ID_LIVE='pri_test_enterprise100';
...
assert.equal('last_paddle_event_occurred_at' in payloadWritten,true);
assert.equal('last_paddle_event_id' in payloadWritten,true);
