-- 03_preview_views_functions_triggers.sql
-- Views + Functions + Triggers (incluye fix tracker geocerca_id)
...
/*
  NOTA IMPORTANTE:
  La función _geojson_extract_geometry(jsonb)
  se define de forma CANÓNICA en:
  20260208000287_preview_prereq__geojson_extract_geometry.sql

  Aquí NO debe redefinirse para evitar:
  - conflictos de return type
  - errores en db push
*/

/* === CONTINÚA EL ARCHIVO NORMALMENTE === */
...
AS $$
  ...
  where t.typname::text = public._organizations_plan_type()::text
...
declare
  v_org_id uuid;
  v_role text;
  v_prev_row_security text;
begin
  -- Guardar estado previo y desactivar RLS solo dentro de esta función
  v_prev_row_security := current_setting('row_security', true);
  ...
  -- Bootstrap-safe: en migraciones / sesiones sin contexto no debe romper
  if v_org_id is null then
    -- Restaurar row_security al estado previo (si existía)
    if v_prev_row_security is not null then
      perform set_config('row_security', v_prev_row_security, true);
    end if;
    return null;
  end if;
...
CREATE MATERIALIZED VIEW IF NOT EXISTS "public"."latest_tracker_position" AS
