...
// - ✅ Debug watermark: confirma que ESTE componente es el que estás viendo
...
export default function AsignacionesPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      const { data: a, error: aErr } = await supabase
        ...
        .select("id, org_id, tracker_user_id, geofence_id, start_date, end_date, active, created_at, updated_at, frequency_minutes, geofence_name, tracker_email, tracker_label, tracker_name")
        .eq("org_id", orgId)
        .order("created_at", { ascending: false });
      if (aErr) throw aErr;
  ...
  async function toggleActive(row) {
    ...
    if (!startDate || !endDate) return setError("Fila con fechas inválidas (start/end).");
    ...
    <div className="p-4 w-full">
      {debug ? (
        <div className="mb-3 border rounded bg-yellow-50 px-3 py-2 text-xs text-yellow-900">
          <div className="font-semibold">DEBUG: AsignacionesPage.jsx (ruta /asignaciones)</div>
          <div>Fuente de lectura: <span className="font-mono">v_tracker_assignments_ui</span></div>
          <div>Rows: {rows?.length || 0} | Org: <span className="font-mono">{orgId}</span></div>
        </div>
      ) : null}
