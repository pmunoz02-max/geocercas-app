...
export default function Reports() {
  ...
  async function loadReport() {
    ...
    try {
      ...
      if (start && end && start > end) throw new Error('La fecha "Desde" no puede ser mayor que la fecha "Hasta".');
    ...
    <div className="space-y-6 max-w-6xl mx-auto p-4 md:p-6">
      ...
      <div className="border rounded-xl bg-white p-4 shadow-sm space-y-4">
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <button onClick={loadReport} disabled={loadingReport} className="px-4 py-2 rounded-lg bg-emerald-700 text-white hover:bg-emerald-800 disabled:opacity-60">
            {loadingReport ? "Generando…" : "Generar"}
          </button>

          <button onClick={() => exportRowsToCSV(rows, "reportes")} disabled={!rows.length} className="px-4 py-2 rounded-lg border hover:bg-slate-100 disabled:opacity-60">
            Exportar CSV
          </button>

          <button onClick={loadFilters} disabled={loadingFilters} className="px-4 py-2 rounded-lg border hover:bg-slate-100 disabled:opacity-60">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select multiple value={selectedGeocercaIds} onChange={onMultiSelectChange(setSelectedGeocercaIds)} className="block w-full border rounded-lg px-2 py-2 mt-1 min-h-[140px]" disabled={loadingFilters}>
          ...
          <div>
            ...
            <select multiple value={selectedPersonalIds} onChange={onMultiSelectChange(setSelectedPersonalIds)} className="block w-full border rounded-lg px-2 py-2 mt-1 min-h-[140px]" disabled={loadingFilters}>
          ...
          <div>
            ...
            <select multiple value={selectedActivityIds} onChange={onMultiSelectChange(setSelectedActivityIds)} className="block w-full border rounded-lg px-2 py-2 mt-1 min-h-[140px]" disabled={loadingFilters}>
          ...
          <div>
            ...
            <select multiple value={selectedAsignacionIds} onChange={onMultiSelectChange(setSelectedAsignacionIds)} className="block w-full border rounded-lg px-2 py-2 mt-1 min-h-[140px]" disabled={loadingFilters}>
      ...
      <section className="overflow-x-auto rounded-xl border bg-white shadow-sm">
        {loadingReport ? (
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={r.attendance_id ? `${r.attendance_id}-${i}` : i} className={`border-t hover:bg-slate-50 ${i % 2 === 0 ? "bg-white" : "bg-slate-50/40"}`}>
                  ...
                  <td className="p-2">{r.asignacion_id ? `${String(r.asignacion_id).slice(0, 8)} (${r.asignacion_status || "—"})` : "—"}</td>
