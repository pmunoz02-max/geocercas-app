...
async function callInviteTrackerAPI(payload) {
  ...
  try {
    res = await fetch(url, {
      method: "POST",
      credentials: "include", // 🔑 manda cookies tg_at/tg_rt
      headers: {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        const serverMsgRaw =
          ...
          (resp?.data?.raw ? resp.data.raw : "") ||
          (resp?.raw ? resp.raw : "");

        const serverMsg =
          typeof serverMsgRaw === "string"
            ? serverMsgRaw
            : serverMsgRaw
      ...
      if (!proxyPayload?.ok) {
        ...
        setMessage({
          type: "error",
          text: typeof innerErr === "string" ? innerErr : JSON.stringify(innerErr),
        });
        return;
      }
