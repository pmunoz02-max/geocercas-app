...
export default async function handler(req, res) {
  const version = "invite-tracker-proxy-esm-v3-2026-01-21";

  try {
    // CORS (same-origin recommended)
    ...
    if (!email || !email.includes("@") || !org_id) {
      res.statusCode = 400;
      res.setHeader("Content-Type", "application/json; charset=utf-8");
      return res.end(JSON.stringify({ ok: false, error: "Missing or invalid fields", required: ["email", "org_id"], version }));
    }
    ...
    if (!accessToken) {
      if (!refreshToken) {
        res.statusCode = 401;
        res.setHeader("Content-Type", "application/json; charset=utf-8");
        return res.end(JSON.stringify({ ok: false, error: "No session token (missing tg_at). Please login again.", version }));
      }
      ...
      res.setHeader("Set-Cookie", [
        ...
        makeCookie("tg_rt", refreshed.refresh_token || refreshToken, { httpOnly: true, secure: true, sameSite: "Lax", path: "/", maxAge: refreshMaxAge }),
      ]);
    }

    const fnUrl = `${String(SUPABASE_URL).replace(/\/$/, "")}/functions/v1/invite_tracker`;

    const fnRes = await nodeRequest(fnUrl, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    res.setHeader("Content-Type", "application/json; charset=utf-8");
    return res.end(JSON.stringify({ ok: fnRes.ok, status: fnRes.status, data: json, version }));
  } catch (fatal) {
