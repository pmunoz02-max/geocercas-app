...
it('shows the photo selector before starting and includes the selected photo in the visit',async()=>{
 ...
 const input=await screen.findByLabelText('Adjuntar foto (JPG o PNG, máximo 2 MB)');
 ...
 await waitFor(()=>expect(putVisitDraft).toHaveBeenCalledWith('org:user',expect.objectContaining({photo:expect.objectContaining({base64:'photo',location:null})})));
