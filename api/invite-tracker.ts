...
const BUILD_TAG = "invite-tracker-v7-uuid-guard";
...
export default async function handler(req: any, res: any) {
  try {
    ...
    // ✅ GUARDIA PERMANENTE: si org_id no es UUID, NO generamos link malo
    if (!isUuid(org_id)) {
      return json(res, 400, {
        ok: false,
        error: "org_id inválido (no es UUID). Revisa currentOrg.id en frontend.",
        value: org_id,
      });
    }
    ...
    // 4) Default logic (membership default org)
    ...
    const magic_link = (linkData as any)?.properties?.action_link || (linkData as any)?.action_link || null;
    ...
    return json(res, 200, {
      ...
      magic_link,
      note: "Link directo listo. Si org_id era inválido antes, ahora se bloquea con 400.",
    });
  } catch (e: any) {
