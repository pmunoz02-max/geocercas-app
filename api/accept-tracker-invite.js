...
// Proxy universal same-origin para accept-tracker-invite (evita CORS).
// - Lee cookies tg_at/tg_rt
// - Llama a /functions/v1/accept-tracker-invite
// - Refresh si 401 invalid jwt

const BUILD_TAG = "accept-invite-proxy-v1-universal-same-origin-20260219";
...
async function callAccept({ supabaseUrl, anonKey, userJwt, orgId, body }) {
  const base = String(supabaseUrl).replace(/\/$/, "") + "/functions/v1/accept-tracker-invite";
...
export default async function handler(req, res) {
  try {
    ...
    let r = await callAccept({ supabaseUrl, anonKey, userJwt: tgAt, orgId, body });
    ...
    if (isInvalidJwt401(r.status, r.json)) {
      ...
      r = await callAccept({ supabaseUrl, anonKey, userJwt: refreshed.access_token, orgId, body });
