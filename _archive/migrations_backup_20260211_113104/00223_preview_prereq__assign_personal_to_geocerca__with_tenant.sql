-- PREREQ bootstrap-safe para permitir GRANT/REVOKE en 00300_preview_rls.sql
-- Redefinida por 00400_preview_vft.sql

create or replace function public.assign_personal_to_geocerca(
  p_tenant_id uuid,
  p_personal_id uuid,
  p_geocerca_id uuid,
  p_start_date date,
  p_end_date date,
  p_freq_min integer
)
returns setof record
language plpgsql
security definer
set search_path = public
as $$
begin
  -- no-op en bootstrap
  return;
end;
$$;

comment on function public.assign_personal_to_geocerca(uuid, uuid, uuid, date, date, integer) is
'PREREQ bootstrap-safe para pasar 00300_preview_rls.sql; redefinida por 00400_preview_vft.sql';
