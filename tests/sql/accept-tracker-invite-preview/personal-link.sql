-- Preview regression: recipient personal linkage, rollback and valid retries.
...
u uuid; n integer; lim integer; plan text; person_id uuid; other_person uuid;
BEGIN
...
INSERT INTO public.personal(org_id,nombre,email,user_id,telefono)
VALUES(o,'Recipient linkage test',email_a,NULL,'+1000000001') RETURNING id INTO person_id;
INSERT INTO public.personal(org_id,nombre,email,user_id,telefono)
VALUES(other_org,'Other organization',email_a,NULL,'+1000000002') RETURNING id INTO other_person;
...
IF EXISTS(SELECT 1 FROM public.personal WHERE id=person_id AND user_id IS NOT NULL) THEN
RAISE EXCEPTION 'Rejected plan/cap linked personal'; END IF;
UPDATE public.personal SET user_id=b WHERE id=person_id;
PERFORM pg_temp.expect_rejection(o,'accept-test-A','invite_identity_mismatch');
UPDATE public.personal SET user_id=NULL WHERE id=person_id;
-- Another personal record already owns the resolved user in this org.
INSERT INTO public.personal(org_id,nombre,email,user_id,telefono)
VALUES(o,'Conflicting user link','different-'||a||'@example.invalid',a,'+1000000003') RETURNING id INTO j;
PERFORM pg_temp.expect_rejection(o,'accept-test-A','invite_identity_mismatch');
DELETE FROM public.personal WHERE id=j;
...
IF EXISTS(SELECT 1 FROM public.personal WHERE id=person_id AND user_id IS NOT NULL) THEN RAISE EXCEPTION 'Personal linkage escaped rollback'; END IF;
...
IF NOT EXISTS(SELECT 1 FROM public.personal WHERE id=person_id AND user_id=a)
 OR EXISTS(SELECT 1 FROM public.personal WHERE id=other_person AND user_id IS NOT NULL) THEN
RAISE EXCEPTION 'Recipient not linked or other org changed'; END IF;
first_time:=r->>'accepted_at';
-- Simulate a historical accepted invitation whose personal link was never set.
UPDATE public.personal SET user_id=NULL WHERE id=person_id;
r:=public.accept_tracker_invite_transactional(o,'accept-test-A',a);
IF r->>'already_accepted'<>'true' OR r->>'accepted_at'<>first_time THEN RAISE EXCEPTION 'Retry changed acceptance'; END IF;
IF NOT EXISTS(SELECT 1 FROM public.personal WHERE id=person_id AND user_id=a) THEN RAISE EXCEPTION 'Valid retry failed to repair link'; END IF;
...
IF EXISTS(SELECT 1 FROM public.personal WHERE id=person_id AND user_id IS NOT NULL) THEN RAISE EXCEPTION 'Revoked retry linked personal'; END IF;
